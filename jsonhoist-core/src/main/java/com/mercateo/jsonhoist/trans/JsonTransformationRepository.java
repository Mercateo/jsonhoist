...
public class JsonTransformationRepository {
    ...
    private final Map<HoistMetaData, List<JsonTransformationStep>> graph = Collections
            .synchronizedMap(
                    new HashMap<>());

    public List<JsonTransformation> defaultPath(@NonNull HoistMetaData from,
            @NonNull HoistMetaData to) {
        ...
        return path.get().stream().map(JsonTransformationStep::getTransformation).collect(Collectors
                .toList());
    }

    private Optional<List<JsonTransformationStep>> path(
            @NonNull List<JsonTransformationStep> journey, @NonNull HoistMetaData from,
            @NonNull HoistMetaData to) {
        ...
        if (options.isPresent()) {
            for (JsonTransformationStep jsonTransformationStep : options.get()) {
                if (!alreadyVisited(journey, jsonTransformationStep)) {
                    ...
                    Optional<List<JsonTransformationStep>> possiblePath = path(newPossibleJourney,
                            transformationTarget, to);
    ...
    protected boolean alreadyVisited(List<JsonTransformationStep> journey,
        ...
        return journey.stream()
                .map(step -> step.getTarget())
                .filter(jsonTransformationStep.getTarget()::equals)
                .findAny()
                .isPresent();
    }
